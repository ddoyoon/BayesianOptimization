...
class PTBModel(object):
    ...
    def import_ops(self):
        ...
        self._cost = tf.get_collection_ref(
            util.with_prefix(self._name, "cost")
        )[0]
        ...
        if verbose and step % (model.input.epoch_size // 10) == 10:
            print(
                ...
                % (
                    ...
                    np.exp(costs / iters),
                    iters
                    * model.input.batch_size
                    * max(1, FLAGS.num_gpus)
...
def get_config():
    ...
    if FLAGS.num_gpus != 1 or StrictVersion(tf.__version__) < StrictVersion(
        "1.3.0"
    ):
...
def train_and_validate(
    ...
    with tf.Graph().as_default():
        ...
        if (
            StrictVersion(tf.__version__) < StrictVersion("1.1.0")
        ...
        with sv.managed_session(config=config_proto) as session:
            for i in range(config.max_max_epoch):
                ...
                train_perplexity = run_epoch(
                    session, m, eval_op=m.train_op, verbose=False
                )
                if FLAGS.verbose:
                    print(
